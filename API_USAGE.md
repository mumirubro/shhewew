...
- **Smart Product Selection**: Automatically finds and uses the cheapest available product on any site
- **Global URL Support**: Set once with `/seturl`, works with `/sh` and `/msh` endpoints
- **Global Proxy Rotation**: Add once with `/addp`, auto-applies to ALL card checks
- **12 REST Endpoints**: Complete control over card checking, URLs, proxies, and site testing
...
### 3. `/msh` - Multiple Cards Check (GLOBAL URL SUPPORTED!)
...
**Format** (with global URL - set via `/seturl`):
```
/msh?cards={cc1},{cc2},{cc3}
```
...
**Example 2** (using global URL - no site parameter needed!):
```
...
## 🔌 Proxy Management Endpoints (GLOBAL - Auto-applies to ALL card checks!)

**Note**: Once you add proxies, they automatically apply to ALL card checking requests (`/shopify.py`, `/sh`, `/msh`). The system rotates through them automatically!

### 7. `/addp` - Add Proxy

**Purpose**: Add a proxy to rotation pool (automatically used for ALL card checks)
