...
def multi_check():
    ...
    if not cards_param:
        return jsonify({
            ...
            'usage': 'GET /msh?cards={cc1},{cc2}&site={url} OR /msh?cards={cc1},{cc2} (with global URL)',
            'example': '/msh?cards=4532|12|25|123,4556|01|26|456&site=https://shop.com',
            'example_with_global_url': '/msh?cards=4532|12|25|123,4556|01|26|456'
        }), 400
    ...
    if not site and not GLOBAL_SETTINGS['url']:
        return jsonify({
            ...
            'usage': 'Provide site parameter OR set global URL with /seturl first',
            'example_with_site': '/msh?cards=4532|12|25|123&site=https://shop.com',
            'example_with_global': 'First use /seturl?url=https://shop.com, then /msh?cards=4532|12|25|123'
        }), 400
...
def check_proxy():
    ...
    async def test_proxy():
        try:
            start_time = time.time()
            async with httpx.AsyncClient(proxies=next_proxy, timeout=15.0) as client:
