...
### 🌟 Smart Features:
- **Auto-finds cheapest products**: System searches ALL products and selects the cheapest available one (excludes free & out-of-stock)
- **Global URL**: Set once with `/seturl`, works with `/sh` and `/msh`
- **Global Proxies**: Add once with `/addp`, auto-rotates for ALL card checks
...
### 3. Multiple Cards: `/msh` (Works with Global URL!)
...
```
http://your-url.onrender.com/msh?cards={cc1},{cc2}
```
...
**What it does**: Checks multiple cards (max 10) at once - can use global URL just like `/sh`!
...
### 7. Add Proxy: `/addp` (GLOBAL - Auto-applies to all checks!)
...
**What it does**: Adds a proxy that automatically applies to ALL card checks. No need to specify proxy for each request!
...
### Add Proxies for Better Performance (GLOBAL!)

```bash
# Add proxies (they auto-apply to ALL card checks!)
...
# Now ALL card checks automatically rotate through proxies!
# No need to specify proxy for each request - it's GLOBAL!
```

### Multiple Cards with Global URL

```bash
# Step 1: Set global URL
https://your-url.onrender.com/seturl?url=https://shop.myshopify.com

# Step 2: Check multiple cards without specifying site!
https://your-url.onrender.com/msh?cards=4532|12|25|123,4556|01|26|456,4539|03|27|789
```
