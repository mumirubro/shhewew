...
**✨ Key Features:**
- 🎯 Automatically finds and uses the **cheapest available product** on any Shopify site
- 🔄 Smart proxy rotation (set once, applies to all requests)
- 🌐 Global URL support (set once, use everywhere)
- 📦 12 REST API endpoints for complete control
...
The system automatically searches **ALL products** on any Shopify site and intelligently selects the cheapest available option:
- ✅ Excludes free ($0) products
- ✅ Excludes out-of-stock items
- ✅ Finds the absolute lowest price among available products
- ✅ Works on ANY Shopify site automatically

### Global Settings
Set once, use everywhere:
- **Global URL**: Set with `/seturl` - applies to `/sh` and `/msh` endpoints
- **Global Proxies**: Add with `/addp` - automatically rotates for ALL card checks
...
| Endpoint | Purpose | Example | Global URL Support |
|----------|---------|---------|-------------------|
| `/shopify.py` | Main card check | `/shopify.py?lista={site}\|{cc}` | ❌ |
| `/sh` | Single card | `/sh?lista={site}\|{cc}` or `/sh?lista={cc}` | ✅ |
| `/msh` | Multiple cards | `/msh?cards={cc1},{cc2}&site={url}` or `/msh?cards={cc1},{cc2}` | ✅ |
...
| Endpoint | Purpose | Example |
|----------|---------|---------|
| `/addp` | Add proxy (applies to all checks) | `/addp?proxy=http://user:pass@ip:port` |
...
**Step 4**: All card checks now automatically rotate through proxies!
```
https://your-app.onrender.com/shopify.py?lista=https://shop.com|4532|12|25|123
```
(This request automatically uses your proxies - no need to specify!)
