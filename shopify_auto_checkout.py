...
class ShopifyAuto:
    ...
    async def get_product_info(self, session, site_url):
        """Get product info - ALWAYS selects the CHEAPEST available product"""
        try:
            all_products = []
            ...
            if response.status_code == 200:
                ...
                if data.get('products'):
                    for prod in data['products']:
                        if not any(p.get('id') == prod.get('id') for p in all_products):
                            all_products.append(prod)
            
            if all_products:
                print(f"\n🔍 Found {len(all_products)} products on site, searching for CHEAPEST...")
                
                cheapest_option = None
                cheapest_price_float = float('inf')
                    ...
                    for variant in product['variants']:
                        ...
                        try:
                            if price is not None:
                                ...
                                if price_float < cheapest_price_float and price_float > 0:
                                    cheapest_price_float = price_float
                                    ...
                                    cheapest_option = {
                                        ...
                                        'price': price_cents,
                                        'available': True,
                                        'price_dollars': price_float
                                    }
                        except (ValueError, TypeError):
                            continue
                
                if cheapest_option:
                    print(f"💰 CHEAPEST PRODUCT FOUND: {cheapest_option['title']}")
                    print(f"💵 LOWEST PRICE: ${cheapest_option['price_dollars']:.2f}")
                    print(f"✅ Selected variant ID: {cheapest_option['id']}")
                    cheapest_option.pop('price_dollars', None)
                    return cheapest_option
                else:
                    print("⚠️ No available paid products found on site, using default fallback")
